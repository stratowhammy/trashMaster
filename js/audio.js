...
class SoundManager {
    ...
    playAlexJonesFaceMeltSFX() {
        if (this.isMuted) return;
        if (!this.ctx) this._initAudio();
        if (!this.ctx) return;
        const now = this.ctx.currentTime;

        // 1. Alarm klaxon bursts
        [0, 0.15, 0.30].forEach((delay) => {
            ...
            g.gain.exponentialRampToValueAtTime(0.01, now + delay + 0.12);
            osc.connect(g);
        ...
        // 2. High-energy laser eyebeam zap
        const beamOsc = this.ctx.createOscillator();
        const beamGain = this.ctx.createGain();
        beamOsc.type = 'sawtooth';
        beamOsc.frequency.setValueAtTime(2400, now + 0.4);
        beamOsc.frequency.exponentialRampToValueAtTime(150, now + 1.6);
        beamGain.gain.setValueAtTime(0.38, now + 0.4);
        beamGain.gain.exponentialRampToValueAtTime(0.001, now + 1.8);
        beamOsc.connect(beamGain);
        beamGain.connect(this.masterGain);
        beamOsc.start(now + 0.4);
        beamOsc.stop(now + 1.8);

        // 3. Molten Face Sizzle / Frying Noise
        try {
            ...
            const filter = this.ctx.createBiquadFilter();
            filter.type = 'bandpass';
            filter.frequency.setValueAtTime(1800, now + 0.5);
            filter.frequency.exponentialRampToValueAtTime(400, now + 2.5);
            filter.Q.setValueAtTime(5.0, now + 0.5);
        ...
        subOsc.type = 'sine';
        subOsc.frequency.setValueAtTime(95, now + 0.4);
        subOsc.frequency.exponentialRampToValueAtTime(25, now + 2.2);
        subGain.gain.setValueAtTime(0.45, now + 0.4);
